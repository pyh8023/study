...
4.创建EventDrivenTaskSource 并调用start()加载Split
...
2. 为所有SplitSource（从数据源表获取数据）分别创建一个TableScanSplitLoader
...
调用createExchangeSplitLoader()为每个ExchangeSplitSource创建SplitLoader
                    ...
                    public void update(ListMultimap<Integer, Split> splits, boolean noMoreSplits)
                    {
                        try {
                            synchronized (assignerLock) {
                                ...
                                if (noMoreSplits) {
                                    //如果对应RemoteSourceNode所有split都完成执行，向finishedSources添加RemoteSourceNode的NodeId
...
调用SplitLoader.start()方法加载Split，start()直接调用processNext()进行加载，执行步骤如下：

1.每次从SplitSource获取splitBatchSize大小的数据
...
private synchronized void processNext()
{
    ...
    //splitBatchSize由query.schedule-split-batch-size配置执行，默认为1000

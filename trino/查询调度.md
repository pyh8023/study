...
这3中纸执行策略对应了ExecutionSchedule的三个实现类：
...
```java
	...
	//将生成的outputBufferId添加到上游Stage的outputBufferManager中，并更新Source Task的OutputBuffer
...
当Stage有下游Stage时，会将下游Stage的TaskLifecycleListener作为构造参数传入
...
```java
    ...
    //当Source Task创建时，会调用该方法，更新当前Stage的SourceTask列表

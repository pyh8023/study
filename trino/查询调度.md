            ...
            while (nodeAcquisitionIterator.hasNext()) {
                ...
                //当任务节点分配完成，创建执行任务
                    ...
                    try {
                        ...
                        //任务创建成功后绑定事件监听器
                        remoteTask.ifPresent(task -> {
                            //当outputBuffer更新时，向事件队列添加RemoteTaskExchangeSinkUpdateRequiredEvent事件
                            ...
                            //添加任务执行完成，向事件队列添加RemoteTaskCompletedEvent事件
        ...
        private void createStageExecution(SubPlan subPlan, boolean rootFragment, int schedulingPriority)
        {
            ...
            try {
                ...
                for (SubPlan child : subPlan.getChildren()) {
                    PlanFragmentId childFragmentId = child.getFragment().getId();
                    StageExecution childExecution = getStageExecution(getStageId(childFragmentId));
                    //每次添加SourceOutputSelecto，会将同一个RemoteSourceNode对应的所有Stage的ExchangeSourceOutputSelector进行合并
...
每个SplitLoader负责一个SplitSource的加载，调用SplitLoader.start()方法加载对应SplitSource的Split，start()直接调用processNext()进行加载，执行步骤如下：

1.每次从SplitSource获取一个SplitBatch，SplitBatch的数据最大大小由query.schedule-split-batch-size配置指定，默认为1000

2.SplitBatch加载完成后，触发EventDrivenTaskSource回调事件，向Scheduler中的事件队列添加相关事件
...
private synchronized void processNext()
{
    ...
    Futures.addCallback(splitLoadingFuture, new FutureCallback<>()
    {
		...
		//splitToPartition::applyAsInt获取split的partitionId
                ...
                //调用构造方法传入的回调事件，触发EventDrivenTaskSource回调事件，向Scheduler中的事件队列添加相关事件
...
StageExecution在EventDrivenFaultTolerantQueryScheduler.StageExecution.schedule()
...
EventDrivenFaultTolerantQueryScheduler.StageExecution.addPartition()
```java
...
##### NoMorePartitionsEvent
NoMorePartitionsEvent事件的处理源码如下：
...
##### PartitionsUpdatedEvent
PartitionsUpdatedEvent事件的处理源码如下：
...
EventDrivenFaultTolerantQueryScheduler.StageExecution.updatePartition()
```java

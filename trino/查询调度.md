...
SourcePartitionedScheduler负责Source Stage的调度。
...
FixedSourcePartitionedScheduler也是负责Source Stage的调度，与SourcePartitionedScheduler区别在于，对应的Stage中除了本地Source，还有RemoteSourceNode。
...
### NodeSelector
NodeSelector提供了各个Stage中Task的分配节点的算法。NodeSelector有两个实现类：TopologyAwareNodeSelector和UniformNodeSelector，由node-scheduler.policy配置决定选择哪个实现类，默认是UniformNodeSelector。
UniformNodeSelector尝试在数据所在的主机分配split，维护所有主机的均匀分布。
TopologyAwareNodeSelector根据节点之间的网络拓扑距离来调度splits。

#### NodeMap
NodeSelector实现类维护了一个NodeMap，NodeMap本身根据节点信息维护了3份不同形式的列表。TopologyAwareNodeSelector和UniformNodeSelector的NodeMap由其对应的工厂类创建。
##### UniformNodeSelector的NodeMap创建
UniformNodeSelectorFactory的createNodeMap()中，通过NodeManager.getActiveCatalogNodes()获取对应catalog的节点，如果为空则通过NodeManager.getNodes(ACTIVE)获取所有Active的节点。然后将对应节点存放到byHostAndPort和byHost两个集合中，目的是在Source Stage分配Split时，实现数据本地性而设置的。
```java
...
```
##### TopologyAwareNodeSelector的NodeMap创建
TopologyAwareNodeSelectorFactory的createNodeMap()中，与UniformNodeSelectorFactory获取节点的方式相同。区别在于TopologyAwareNodeSelectorFactory多设置了workersByNetworkPath集合，存储了不同网络路径对应的节点。
```java
    private NodeMap createNodeMap(Optional<CatalogHandle> catalogHandle)
    {
        Set<InternalNode> nodes = catalogHandle
...
Single和Fixed Stage都是随机选择节点，且调用的方法都是NodeSelector.selectRandomNodes()，区别在于Single Stage只随机从所有存活节点列表中选择一个节点，而Fixed Stage则随机选择多个节点，最多选择节点数由query.initial-hash-partitions配置或hash_partition_count session指定，默认为100，如果存活节点的数量少于该配置，则返回所有存活节点。

Single和Fixed Stage是通过NodePartitioningManager.getNodePartitioningMap()选择节点，对应源码如下：
```java
 ...
 ```
#### Source Stage节点选择策略
SourcePartitionedScheduler和FixedSourcePartitionedScheduler都是Source Stage的调度器，两者的调度策略不同。
SourcePartitionedScheduler会根据Split的本地性进行调度，调用的是而FixedSourcePartitionedScheduler是基于预先计算的固定分区运行指定split的节点。

##### SourcePartitionedScheduler节点选择策略
SourcePartitionedScheduler通过调用NodeSelector.computeAssignments(Set<Split> splits, List<RemoteTask> existingTasks)来为Split选择节点。
	
**UniformNodeSelector的节点选择策略**
	
1.先判断是否开启优化本地性调度策略，如果开启则根据本地性策略性选择候选节点，即调用NodeScheduler.selectExactNodes() ，获取候选节点后，过滤出空闲节点，再从中选择TotalSplitsWeight最小的节点作为运行节点
	
2.如果没开启优化本地性调度策略或没选出节点，则判断split是否远程可访问，如果不可访问，则根据本地性策略性选择候选节点，否则随机选择节点加入候选列表，候选节点数不超过node-scheduler.min-candidates（默认为10）或可选节点数总数
	
3.从候选节点选择运行节点：
  先从候选节点中过滤出空闲节点（节点TotalSplitsWeight小于maxSplitsWeightPerNode，UnacknowledgedSplitCountForStage小于maxUnacknowledgedSplitsPerTask）
	
  然后根据split平衡策略选择节点，如果node-scheduler.splits-balancing-policy配置为STAGE，选择Stage QueuedSplitsWeight最小的节点运行split；如果node-scheduler.splits-balancing-policy为Node，选择TotalSplitsWeight最小的节点运行split

4.如果上一步没选择出节点，选择Stage QueuedSplitsWeight小于adjustedMaxPendingSplitsWeightPerTask且UnacknowledgedSplitCountForStage小于maxUnacknowledgedSplitsPerTask的Stage QueuedSplitsWeight最小的节点
...
```java
    public SplitPlacementResult computeAssignments(Set<Split> splits, List<RemoteTask> existingTasks)
    {
        ...
        //判断是否开启优化本地性调度策略，如果开启则根据本地性策略选择候选节点，即调用NodeScheduler.selectExactNodes()
        if (optimizedLocalScheduling) {
            for (Split split : splits) {
                //如果split远程可访问，且split中地址信息
                if (split.isRemotelyAccessible() && !split.getAddresses().isEmpty()) {
                    //根据本地性策略性选择候选节点
                    //先选择跟split主机和端口匹配的节点加入候选列表，然后再选择主机匹配的节点
                    //如果前面没选出候选列表，并且node-scheduler.include-coordinator为false，则选择coordinator节点
                    List<InternalNode> candidateNodes = selectExactNodes(nodeMap, split.getAddresses(), includeCoordinator);

                    //过滤出空闲节点，再从中选择TotalSplitsWeight最小的节点作为运行节点
        ...
        for (Split split : remainingSplits) {
            ...
            //如果split远程不可访问，则根据本地性策略选择候选节点
            if (!split.isRemotelyAccessible()) {
                candidateNodes = selectExactNodes(nodeMap, split.getAddresses(), includeCoordinator);
            }
            else {
                //随机选择节点加入候选列表，候选节点数不超过node-scheduler.min-candidates（默认为10）或可选节点数总数
            ...
            //1.从候选节点中过滤出空闲节点（节点TotalSplitsWeight小于maxSplitsWeightPerNode，UnacknowledgedSplitCountForStage小于maxUnacknowledgedSplitsPerTask）
            //2.根据split平衡策略选择节点，如果node-scheduler.splits-balancing-policy配置为STAGE，选择Stage QueuedSplitsWeight最小的节点运行split；
            //  如果node-scheduler.splits-balancing-policy配置为Node，选择TotalSplitsWeight最小的节点运行split
            InternalNode chosenNode = chooseNodeForSplit(assignmentStats, candidateNodes);
            //如果没选择出节点，选择Stage QueuedSplitsWeight小于adjustedMaxPendingSplitsWeightPerTask且UnacknowledgedSplitCountForStage小于maxUnacknowledgedSplitsPerTask的Stage QueuedSplitsWeight最小的节点
            if (chosenNode == null) {
...
```
**TopologyAwareNodeSelector的节点选择策略**
	
TopologyAwareNodeSelectorFactory多设置了workersByNetworkPath集合，存储了不同网络路径对应的节点，集合的Key为NetworkLocation，代表节点的网络位置

TopologyAwareNodeSelector的节点选择策略如下：
	
1.如果split不可远程访问，则根据本地性策略选择候选节点，然后调用bestNodeSplitCount()从候选节点选择运行节点
	
2.定位split所在主机的NetworkLocation，如果split地址信息为空，则以ROOT_LOCATION为NetworkLocation，优先选择网络距离最近的节点作为候选节点，再调用bestNodeSplitCount()从候选节点选择运行节点
  
 例如，当node-scheduler.network-topology.segments是region，rack,machine时，先选择region，rack,machine都相同的节点做候选节点，没有的话，再选择region，rack相同的，没有的话，再选择region相同的,还没有的话，选择所有节点，如果split地址信息为空，也是选择所有节点作为候选节点
    ...
    public SplitPlacementResult computeAssignments(Set<Split> splits, List<RemoteTask> existingTasks)
    {
        ...
        for (Split split : splits) {
            ...
            if (!split.isRemotelyAccessible()) {
                ...
                //minCandidates由node-scheduler.min-candidates配置指定，默认为10
                //传给bestNodeSplitCount()的minPendingSplitsWeightPerTask参数为maxPendingSplitsWeightPerTask
            ...
            //depth为网络拓扑层级深度
            //topologicalSplitCounters为List<CounterStat>类型，每个网络层级对应一个CounterStat，用于记录对应网络层级分配的split数量，例如region，rack,machine，三个分别对应一个CounterStat，同时还加上了一个all的CounterStat，这是默认的全局segment
            int depth = topologicalSplitCounters.size() - 1;
            int chosenDepth = 0;

            //获取split所在节点的NetworkLocation
            Set<NetworkLocation> locations = new HashSet<>();
            for (HostAddress host : split.getAddresses()) {
                locations.add(networkTopology.locate(host));
            }
            //如果split的地址为空，则添加ROOT_LOCATION，depth设置为0
            if (locations.isEmpty()) {
                locations.add(ROOT_LOCATION);
                depth = 0;
            }
            //优先选择网络距离最近的节点作为候选节点，再调用bestNodeSplitCount()选择运行节点
            for (int i = depth; i >= 0 && chosenNode == null; i--) {
                for (NetworkLocation location : locations) {
                    ...
                    //segments代表着网络拓扑结构的不同层级，比如网络位置为/region/rack/machine, segments则为region，rack,machine三者组成的List
                    ...
                    //获取当前网络深度的所有节点
                    //workersByNetworkPath集合，存储了不同网络深度地址对应的节点，集合的Key为NetworkLocation，代表节点的网络位置			     
                    Set<InternalNode> nodes = nodeMap.getWorkersByNetworkPath().get(location);
                    //minCandidates由node-scheduler.min-candidates配置指定，默认为10
                    //传给bestNodeSplitCount()的minPendingSplitsWeightPerTask参数是根据所选节点离split的网络拓扑距离来计算的，距离越远minPendingSplitsWeightPerTask更小，最小为maxPendingSplitsWeightPerTask的一半，地址安全匹配的为maxPendingSplitsWeightPerTask
                    ...
                    //如果当前网络深度的地址资源不够则添加到filledLocations中
...
bestNodeSplitCount()从候选节点选择运行节点的策略如下：
	
1.遍历候选节点，如果节点空闲(节点splitWeight+TotalSplitsWeight小于maxSplitsWeightPerNode，splitWeight+UnacknowledgedSplitCountForStage小于maxUnacknowledgedSplitsPerTask)，则直接选择其运行split
	
2.上一步不满足的话，选择splitWeight+节点TotalSplitsWeight < minPendingSplitsWeightPerTask且QueuedSplitsWeightForStage最小的节点	
...
```java
    private InternalNode bestNodeSplitCount(SplitWeight splitWeight, Iterator<InternalNode> candidates, int minCandidatesWhenFull, long minPendingSplitsWeightPerTask, NodeAssignmentStats assignmentStats)
    {
        ...
        //当没有Queue未满的节点，或者已遍历的候选节点没超过限制时，才会继续遍历
        while (candidates.hasNext() && (fullCandidatesConsidered < minCandidatesWhenFull || bestQueueNotFull == null)) {
            ...
            //如果节点UnacknowledgedSplitCountForStage >= maxUnacknowledgedSplitsPerTask,直接跳过当前节点
            if (assignmentStats.getUnacknowledgedSplitCountForStage(node) >= maxUnacknowledgedSplitsPerTask) {
                fullCandidatesConsidered++;
                continue;
            }
            //直接选择splitWeight+节点TotalSplitsWeight < maxSplitsWeightPerNode的节点
            if (canAssignSplitBasedOnWeight(assignmentStats.getTotalSplitsWeight(node), maxSplitsWeightPerNode, splitWeight)) {
                return node;
            }
            fullCandidatesConsidered++;

            //选择splitWeight+节点TotalSplitsWeight < minPendingSplitsWeightPerTask且QueuedSplitsWeightForStage最小的节点
...
##### FixedSourcePartitionedScheduler节点选择策略
SourcePartitionedScheduler通过调用NodeSelector.computeAssignments(Set<Split> splits, List<RemoteTask> existingTasks, BucketNodeMap bucketNodeMap)来为Split选择节点。UniformNodeSelector和TopologyAwareNodeSelector的实现都是调用的NodeScheduler.selectDistributionNodes()

NodeScheduler.selectDistributionNodes()选择节点的策略:
从bucketNodeMap获取split对应的节点，如果选择节点UnacknowledgedSplitCountForStage < maxUnacknowledgedSplitsPerTask 且 满足 节点TotalSplitsWeight+splitWeight < maxSplitsWeightPerNode 或 QueuedSplitsWeightForStage + splitWeight < minPendingSplitsWeightPerTask其中一个条件，则将该节点作为运行节点，否则将节点加入到阻塞节点中
...
```java
            ...
            BucketNodeMap bucketNodeMap)
    {
        ...
        for (Split split : splits) {
            //桶到节点映射指定节点放置
            //split到节点映射由对应连接器的ConnectorNodePartitioningProvider实现类提供
            InternalNode node = bucketNodeMap.getAssignedNode(split);
            SplitWeight splitWeight = split.getSplitWeight();

            //如果选择节点空闲，则将该节点作为运行节点
    ...
    {
	//如果选择节点UnacknowledgedSplitCountForStage < maxUnacknowledgedSplitsPerTask 且 满足 节点TotalSplitsWeight+splitWeight < maxSplitsWeightPerNode 或 QueuedSplitsWeightForStage + splitWeight < minPendingSplitsWeightPerTask其中一个条件，则任务节点空闲
...
##### PartitionsSealedEvent
生成PartitionsSealedEvent需要满足下面条件之一：

1.Stage的所有Source的Split都加载完成

2.使用ArbitraryDistributionSplitAssigner时，所有replicatedSources的Split加载完成
...
上述两个事件在EventDrivenFaultTolerantQueryScheduler.Scheduler.processNodeAcquisitions()方法中为RemoteTask绑定事件

##### RemoteTaskExchangeSinkUpdateRequiredEvent

RemoteTask的ExchangeSinkInstanceHandle需要更新时触发
...
##### RemoteTaskCompletedEvent
RemoteTaskCompletedEvent事件的处理步骤如下：

1.获取任务状态，如果任务成功完成，更新StageExecution中的exchange和任务

2.如果任务执行失败，判断是否符合任务重试条件，符合条件则重新调度任务

3.更新下游Stage的ExchangeSourceOutputSelector
...
```java
        ...
        public void onRemoteTaskCompleted(RemoteTaskCompletedEvent event)
        {
            ...
            //更新下游Stage的ExchangeSourceOutputSelector
...
EventDrivenFaultTolerantQueryScheduler.StageExecution.taskFinished()负责处理成功完成的任务，执行步骤如下：

1.完成exchange对应任务的输出

2.将taskId从StagePartition的运行任务中移除，取消StagePartition中的所有RemoteTask

3.向sinkOutputSelectorBuilder添加完成的Task

4.如果整个Stage执行完成，完成exchange的输出，并创建最终的ExchangeSourceOutputSelector
...
```java
        public void taskFinished(TaskId taskId, TaskStatus taskStatus)
        {
            ...
            //完成exchange对应的ExchangeSinkHandle输出
            exchange.sinkFinished(partition.getExchangeSinkHandle(), taskId.getAttemptId());
            //将taskId从StagePartition的运行任务中移除，取消StagePartition中的所有RemoteTask，并返回任务统计信息
            ...
            //向sinkOutputSelectorBuilder添加完成的Task
            sinkOutputSelectorBuilder.include(exchange.getId(), taskId.getPartitionId(), taskId.getAttemptId());
            
            //如果整个Stage执行完成，完成exchange的输出，并创建最终的ExchangeSourceOutputSelector
            if (noMorePartitions && remainingPartitions.isEmpty() && !stage.getState().isDone()) {
                ...
                //创建最终的ExchangeSourceOutputSelector
                ...
                //更新Stage状态为Finished，取消Stage中的所有RemoteTask
                stage.finish();
            }
        }

```
EventDrivenFaultTolerantQueryScheduler.StageExecution.taskFailed()负责处理执行失败的任务，执行步骤如下：

1.将taskId从StagePartition的运行任务中移除，并将剩余重试次数减1

2.如果任务失败原因是OutOfMemoryError，且无法分配足够的内存执行新任务，则将Stage设置为Failed，且不会重试任务

3.如果剩余重试次数为0或者是用户错误，则将Stage设置为Failed，且不会重试任务

4.如果StagePartition没有seal，则不会重试任务

5.如果上述3步都不满足，则创建ScheduledTask，重新调度任务
...
```java
        public List<ScheduledTask> taskFailed(TaskId taskId, ExecutionFailureInfo failureInfo, TaskStatus taskStatus)
        {
            ...
            int partitionId = taskId.getPartitionId();
            StagePartition partition = getStagePartition(partitionId);
            //将taskId从StagePartition的运行任务中移除，并将剩余重试次数减1
            ...
            //如果任务失败原因是OutOfMemoryError，且无法分配足够的内存执行新任务，则将Stage设置为Failed，且返回空任务列表
            ...
            //如果剩余重试次数为0或者是用户错误，将Stage设置为Failed，并返回空任务列表
            ...
            //如果StagePartition没有seal，返回空任务列表
...
SplitAssigner负责Split的放置，并在放置过程中触发PartitionsAddedEvent、NoMorePartitionsEvent、PartitionsUpdatedEvent或PartitionsSealedEvent事件的生成。

SplitAssigner有SingleDistributionSplitAssigner、HashDistributionSplitAssigner和ArbitraryDistributionSplitAssigner实现类。

在创建EventDrivenTaskSource时，会调用EventDrivenTaskSourceFactory.createSplitAssigner()创建SplitAssigner。该方法会根据Stage的partitioning选择不同的SplitAssigner实现类，每个Stage对应一个SplitAssigner。

EventDrivenTaskSourceFactory.createSplitAssigner()的源码如下：
...
```java
            ...
            int maxArbitraryDistributionTaskSplitCount)
    {
        ...
        //partitionedRemoteSources中为ExchangeType类型不是REPLICATE的RemoteSourceNode节点ID
        Set<PlanNodeId> partitionedRemoteSources = fragment.getRemoteSourceNodes().stream()
                .filter(node -> node.getExchangeType() != REPLICATE)
                .map(PlanNode::getId)
                .collect(toImmutableSet());
        //partitionedSources为当前Stage所有TableScanNode的节点ID
        Set<PlanNodeId> partitionedSources = ImmutableSet.<PlanNodeId>builder()
                ...
                .build();
        //replicatedSources中为ExchangeType类型是REPLICATE的RemoteSourceNode节点ID
        Set<PlanNodeId> replicatedSources = fragment.getRemoteSourceNodes().stream()
                .filter(node -> node.getExchangeType() == REPLICATE)
                .map(PlanNode::getId)
                .collect(toImmutableSet());

        //如果Stage为Single或者Coordinator_only类型，则返回SingleDistributionSplitAssigner
        ...
        //如果partitioning为FIXED_ARBITRARY_DISTRIBUTION，SCALED_WRITER_ROUND_ROBIN_DISTRIBUTION或SOURCE_DISTRIBUTION类型，则返回ArbitraryDistributionSplitAssigner
        ...
        //如果partitioning为FIXED_HASH_DISTRIBUTION或SCALED_WRITER_HASH_DISTRIBUTION类型，CatalogHandle不为空，
        //或者partitioning的ConnectorHandle是MergePartitioningHandle的实例，则返回HashDistributionSplitAssigner

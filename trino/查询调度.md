...
### Scheduler查询调度
EventDrivenFaultTolerantQueryScheduler.Scheduler.run()为查询调度入口，源码如下：
```java
        public void run()
        {
            ...
            try {
	        //先进行调度，如果没有调度完成，则进入循环
                if (schedule()) {
...
processEvents()负责事件处理，从事件队列eventQueue获取事件，按照FIFO顺序处理事件。
...
updateStageExecutions()负责StageExecution的创建和更新。
...
```java
		...
		//当subPlan可执行，且StageExecution还未创建时，创建StageExecution
...
scheduleTasks()负责Task的调度，即为Task分配执行节点。
...
processNodeAcquisitions()负责创建执行任务，执行步骤如下：
...
1.创建SourceExchange 估算输出大小

2.创建EventDrivenTaskSource 并调用start()
